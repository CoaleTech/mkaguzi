...
export const useVATReconciliationStore = defineStore(
	"vatReconciliation",
	() => {
		// State
		...
		const uploadFile = async (reconciliationId, fileType, file) => {
			try {
				...
				if (data.message?.success) {
					...
					throw new Error(data.message?.message || data.exc || "Upload failed")
				}
			} catch (err) {
		...
		const summaryStats = computed(() => {
			...
			return {
				...
				totalUnmatchedA:
					activeReconciliation.value.total_unmatched_source_a || 0,
				totalUnmatchedB:
					activeReconciliation.value.total_unmatched_source_b || 0,
				totalDiscrepancies:
