...
const statsCards = computed(() => {
	...
	return [
		...
		},
		{
			...
			filterKey: "status",
			filterValue: "Queued",
			active: props.activeFilter === "Queued",
		},
		{
			label: "Running",
			value: running,
			...
			filterKey: "status",
			filterValue: "Running",
			active: props.activeFilter === "Running",
		},
		{
			label: "Completed",
			value: completed,
		...
		},
		{
			...
			filterKey: "status",
			filterValue: "Failed",
			active: props.activeFilter === "Failed",
		},
		{
			label: "Paused",
			value: paused,
			...
			filterKey: "status",
			filterValue: "Paused",
			active: props.activeFilter === "Paused",
		},
	]
})
	...
	return statuses
		.map((name) => ({
			...
			percentage: Math.round(
				(execs.filter((e) => e.status === name).length / total) * 100,
			),
...
const avgDuration = computed(() => {
	...
	const avg =
		completed.reduce((sum, e) => sum + e.execution_time_ms, 0) /
		completed.length
...
const avgThroughput = computed(() => {
	...
	return Math.round(
		completed.reduce((sum, e) => sum + e.records_per_second, 0) /
			completed.length,
	)
})
...
const totalRecords = computed(() => {
	return props.executions.reduce(
		(sum, e) => sum + (e.total_records_processed || 0),
		0,
	)
})
