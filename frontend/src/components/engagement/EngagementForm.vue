...
const validateTeamMember = (data, allRows = [], currentIndex = null) => {
	...
	if (data.planned_hours < 0)
		errs.planned_hours = "Planned hours cannot be negative"
	if (data.actual_hours < 0)
		errs.actual_hours = "Actual hours cannot be negative"
...
const validateContact = (data, allRows = [], currentIndex = null) => {
	...
	if (data.is_primary && allRows.length > 0) {
		const otherPrimary = allRows.filter(
			(row, idx) =>
				row.is_primary &&
...
const getSectionStatus = (sectionId) => {
	switch (sectionId) {
		case "basic":
			return form.engagement_title && form.audit_type && form.audit_universe
				? "complete"
				: form.engagement_title || form.audit_type
					? "partial"
					: "incomplete"
		case "scope":
			return form.audit_objectives && form.audit_scope
				? "complete"
				: form.audit_objectives || form.audit_scope
					? "partial"
					: "incomplete"
		case "methodology":
		...
		case "timeline":
			return form.period_start && form.period_end
				...
				: form.period_start || form.period_end
					...
					: "incomplete"
		case "team":
			return form.lead_auditor && form.audit_team.length > 0
				? "complete"
				: form.lead_auditor || form.audit_team.length > 0
					? "partial"
					: "incomplete"
		case "contacts":
			return form.key_contacts.length > 0 ? "complete" : "incomplete"
		case "data":
			return form.data_periods.length > 0 ||
				form.bc_data_requirements.length > 0
				? "complete"
				: "incomplete"
		case "documentation":
...
const validateForm = () => {
	...
	if (!form.engagement_title)
		errs.engagement_title = "Engagement title is required"
	...
	if (!form.audit_objectives)
		errs.audit_objectives = "Audit objectives are required"
