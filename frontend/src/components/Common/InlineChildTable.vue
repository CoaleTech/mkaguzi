...
function inputProps(column) {
	...
	// Handle phone field (Data field that might be used for phone)
		...
		fieldType === "data" &&
		(column.key?.toLowerCase().includes("phone") ||
	...
	return schema.fields
		.filter((f) => {
			...
			const isDisplayField =
				f.read_only &&
				f.fieldname.endsWith("_name") &&
				schema.fields.some(
					(field) =>
						field.fieldtype.toLowerCase() === "link" &&
						`${field.fieldname}_name` === f.fieldname,
				)
		...
		.map((f) => {
			...
			if (fieldtype === "link") {
				...
				const hasDisplayField = schema.fields.some(
					(field) => field.fieldname === displayFieldName,
				)
				if (hasDisplayField) {
	...
	rows.forEach((row, idx) => {
		effectiveColumns.value.forEach((col) => {
			...
			if (col.required && (row[col.key] === "" || row[col.key] == null)) {
				validationErrors.value.push(`Row ${idx + 1} - ${col.label}: Required`)
			}
			// custom validator
		...
		if (props.validate) {
			...
			if (res) {
				Object.entries(res).forEach(([fld, message]) => {
					...
					validationErrors.value.push(`Row ${idx + 1} - ${label}: ${message}`)
				})
			}
		}
	})
		...
		const resp = await frappe.call({
			...
			args: {
				...
				fields: [
					"name",
					"title",
					...((column.fetchMap && Object.values(column.fetchMap)) || []),
				],
				limit: 10,
			},
		})
